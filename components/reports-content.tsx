...
export default function ReportsContent({
    ...
    switch (dateFilter) {
      case "last-7-days":
        ...
        filtered = events.filter((event) =>
          isAfter(new Date(event.createdAt), sevenDaysAgo)
        );
        break;
      case "last-30-days":
        const thirtyDaysAgo = subDays(now, 30);
        filtered = events.filter((event) =>
          isAfter(new Date(event.createdAt), thirtyDaysAgo)
        );
        break;
      case "upcoming":
        filtered = events.filter((event) =>
          isAfter(new Date(event.datetime), now)
        );
        break;
      case "past":
        filtered = events.filter((event) =>
          isBefore(new Date(event.datetime), now)
        );
        break;
      default:
        filtered = events;
  ...
  return (
    <div className="p-4 -mt-8 sm:p-6 lg:p-8 space-y-6 bg-milk ">
      {/* Header */}
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {userStats.reduce((sum, stat) => sum + stat._count.role, 0)}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <Tabs defaultValue="events" className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 h-auto">
          ...
          <TabsTrigger value="rsvps" className="text-xs sm:text-sm px-2 py-2">
            Recent RSVPs
          </TabsTrigger>
          {userRole === "ADMIN" && (
          ...
          <Card>
            <CardHeader>
              <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
                <div className="space-y-1">
                  <CardTitle className="text-lg text-orange sm:text-xl">
                    Events Overview
                  </CardTitle>
                  <CardDescription className="text-sm">
                    Detailed view of all events with filtering options
                  </CardDescription>
                </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-start">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-medium truncate text-orange">
                            {event.title}
                          </h3>
        ...
        <TabsContent value="performance" className="space-y-4 mt-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center text-orange text-lg sm:text-xl">
                <BarChart3 className="w-5 h-5 mr-2" />
                Top Performing Events
              </CardTitle>
              <CardDescription>
                Events with the highest RSVP counts
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="rsvps" className="space-y-4 mt-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-lg text-orange sm:text-xl">Recent RSVPs</CardTitle>
              <CardDescription>
                Latest event registrations across all events
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0 sm:p-6">
              {/* Mobile Card View */}
        ...
        {userRole === "ADMIN" && (
          <TabsContent value="users" className="space-y-4 mt-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center text-lg text-orange sm:text-xl">
